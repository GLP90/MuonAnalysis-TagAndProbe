...
process.TFileService = cms.Service("TFileService", fileName = cms.string("tnpZ_MCNLO.root"))
...
if True: # enable and do cmsRun tp_from_aod_MC.py /eos/path/to/run/on [ extra_postfix ] to run on all files in that eos path 
    ...
    if scenario:
        if scenario.startswith("/"):
            ...
            import os.path
            process.TFileService.fileName = "tnpZ_MCNLO_%s.root" % os.path.basename(scenario)
        else:
            process.TFileService.fileName = "tnpZ_MCNLO_%s.root" % scenario
    if len(args) > 1:
